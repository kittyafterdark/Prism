...
function pendingReviewGroups(config) {
  ...
  return [...groups.values()]
    ...
    .filter((group) => {
      ...
      if (group.inferredName && !plausibleInferredSceneName(group.inferredName, { known: Boolean(known), strong: group.strongEvidence })) return false;
      const weakSingle = !group.strongEvidence && group.independentCount < 2;
      if (weakSingle && ['alias-suggestion', 'speaker-conflict', 'new-speaker', 'known-speaker-color', 'unknown-color'].includes(group.kind)) return false;
      return true;
    })
...
async function resolveObservationGroup(payload, userId) {
  ...
  if (!binding) {
    ...
    const manualId = matchingManual?.id || (
      mergeTargetId.startsWith('manual:') ? mergeTargetId : `manual:${hashString(normalizeName(name)).toString(36)}`
    );
...
function provisionalRegistryHints(config, registry) {
  ...
  for (const group of candidates) {
    ...
    if (!current || score > currentScore) bestByName.set(key, group);
  }

  const ownersByColor = new Map();
  for (const group of bestByName.values()) {
    if (!ownersByColor.has(group.observedColor)) ownersByColor.set(group.observedColor, []);
  ...
  const compose = () => {
    ...
    if (discoveryEnabled) {
      lines.push(
        ...
        'A provisional hint is continuity awaiting human approval, not confirmed identity. Following it does not increase its confidence.',
      );
