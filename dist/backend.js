...
function decodeProjectionEntity(source, index) {
    ...
    if (!Number.isFinite(code) || code <= 0)
        return null;
    try {
        return { value: String.fromCodePoint(code), length: numeric[0].length };
    }
    catch {
        return null;
    }
...
function visibleSourceProjection(content, stripMarkdown = false) {
    ...
    for (let index = 0; index < source.length;) {
        ...
        if (escapedTag) {
            index += escapedTag[0].length;
            continue;
        }
        const bbcode = rest.match(/^\[\/?color(?:\s*=\s*[^\]]+)?\]/i);
        if (bbcode) {
            index += bbcode[0].length;
            continue;
        }
        if (source[index] === '<') {
            const end = source.indexOf('>', index + 1);
            if (end >= 0 && end - index <= 1200) {
                index = end + 1;
                continue;
            }
        }
        ...
        if (entity) {
            for (const char of entity.value) {
                chars.push(char);
                map.push({ start: index, end: index + entity.length });
            }
            index += entity.length;
            continue;
        }
        if (stripMarkdown && /[*_~`]/.test(source[index])) {
            let end = index + 1;
            while (end < source.length && source[end] === source[index] && end - index < 3)
                end += 1;
            if (end - index <= 3) {
                index = end;
                continue;
...
function locateVisibleQuote(content, quote, locator = {}) {
    ...
    for (const stripMarkdown of [false, true]) {
        ...
        if (!chosen) {
            ...
            if (occurrenceIndex < candidates.length)
                chosen = candidates[occurrenceIndex];
...
function recolorExactEnclosingColorTag(content, range, quote, color) {
    ...
    while ((match = pattern.exec(source))) {
        ...
        if (range.start < innerStart || range.end > innerEnd)
            continue;
...
function bakeQuoteMarkup(content, payload, color) {
    ...
    const source = String(content || '');
    if (!normalized)
        return { content: source, changed: false, status: 'no-color' };
...
async function bakeManualCorrection(payload, config, chat, userId) {
    ...
    if (!payload?.speakerKey || kind === 'ignored')
        return { status: 'not-applicable' };
    const binding = bindingForSpeakerKey(config, payload.speakerKey);
    const color = bindingRegistryColor(binding);
    if (!binding || !color)
        return { status: 'speaker-has-no-color' };
