...
.ldc-shell[data-prism-layout=tabs] .ldc-main{grid-template-columns:minmax(0,1fr)!important;grid-template-rows:auto minmax(0,1fr);margin-top:6px}
...
.ldc-shell[data-prism-layout=tabs] .ldc-panel{padding:12px 14px 10px}
...
export function setup(ctx) {
    ...
    function applyModalPresentation(layout = resolvedModalLayout(), dimensions = modalDimensions()) { if (!modal)
        ...
        Object.assign(modal.root.style, { display: 'flex', flexDirection: 'column', height: '100dvh', maxHeight: 'none', minHeight: '0', width: '100dvw', maxWidth: 'none', overflow: 'hidden' });
    }
    else {
    ...
    async function openPalette(resetScroll = true) {
        ...
        modal = handle;
        applyModalPresentation(resolvedModalLayout(), dimensions);
    ...
    async function closePalette() { clearTimeout(saveTimer); const shouldSave = saveStatus === 'pending'; ++saveGeneration; if (shouldSave && modal && state?.ok) {
        ...
        await saveEditor(activeTab === 'persona' ? 'persona' : 'character', false);
        setSaveStatus('saved');
    } const handle = modal; handle?.dismiss(); if (modal === handle)
        modal = null; }
