...
export function setup(ctx) {
    ...
    function createFullscreenModalProxy(handle) { releaseFullscreenOverlay(); const overlay = document.createElement('div'); overlay.className = 'ldc-fullscreen-root'; overlay.setAttribute('role', 'dialog'); overlay.setAttribute('aria-modal', 'true'); overlay.setAttribute('aria-label', 'Prism'); copyThemeToOverlay(handle.root, overlay); document.body.appendChild(overlay); fullscreenOverlay = overlay; modalHostHandle = handle; handle.root.classList.add('ldc-fullscreen-host-hidden'); bodyOverflowBefore = document.body.style.overflow; document.body.style.overflow = 'hidden'; const proxy = { root: overlay, dismiss() { releaseFullscreenOverlay(); handle.dismiss(); }, onDismiss(callback) { return handle.onDismiss(callback); } }; handle.onDismiss(() => releaseFullscreenOverlay()); return proxy; }
    function applyModalPresentation(layout = resolvedModalLayout(), dimensions = modalDimensions()) { if (!modal)
        ...
        Object.assign(modal.root.style, { display: 'flex', flexDirection: 'column', height: '100dvh', maxHeight: '100dvh', minHeight: '0', width: '100%', maxWidth: '100%', overflow: 'hidden' });
    }
    else {
    ...
    function render() {
        ...
        const missing = missingCount(), engine = normalizeEngine(state.config.engine), needs = missing > 0, ui = uiPreferences(), expanded = ui.modalExpanded === true, ultrawide = isUltrawideViewport(), modalLayout = resolvedModalLayout(), effectiveUiScale = detectedUiScale();
    ...
    function wire() {
        ...
        modal.root.querySelectorAll('[data-character-id]').forEach(row => { const activate = e => { if (e.target.closest('input,[data-inline-swatch]'))
            ...
            sideScrollLeft = roster.scrollLeft; navigateEditor(() => { selectedId = row.dataset.characterId; }); }; row.onclick = activate; row.onkeydown = e => { if (e.key === 'Enter' || e.key === ' ')
            activate(e); }; });
