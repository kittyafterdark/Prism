...
export function setup(ctx) {
    ...
    const unsubBackend = ctx.onBackendMessage((payload) => {
        ...
        if (payload.type === 'ldc_error')
            task.reject(new Error(payload.error || 'Unknown error'));
        else
    ...
    function setBusy(value) {
        ...
        modal?.root.querySelectorAll('button,input,select').forEach((element) => {
            if (element.dataset.allowBusy === 'true')
                return;
    ...
    function characterPanel(character) {
        if (!character)
            return '<div class="ldc-empty">Cortex has not registered a character in this scene.</div>';
    ...
    function render() {
        ...
        if (!selectedCharacterId && state.characters?.length)
    ...
    function wireModal() {
        ...
        modal.root.querySelector('[data-action="bind-persona"]')?.addEventListener('click', () => perform(async () => {
            ...
            if (!persona || !color)
                throw new Error('Select a persona and enter a valid six-digit hex color.');
            const response = await request('ldc_save_binding', {
                chatId: state.chat.id,
                kind: 'persona',
                targetId: persona.id,
        ...
        try {
            ...
            for (const { element } of ctx.dom.listMessageElements()) {
                ...
                if (element.dataset.part === 'user') {
                    if (!personaColor || state.config.autoUserMode === 'off')
                        continue;
