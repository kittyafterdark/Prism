...
.ldc-portal-dialog-backdrop{position:fixed;inset:0;z-index:2147483647;display:grid;place-items:center;padding:max(14px,env(safe-area-inset-top)) max(14px,env(safe-area-inset-right)) max(14px,env(safe-area-inset-bottom)) max(14px,env(safe-area-inset-left));background:rgba(0,0,0,.66);backdrop-filter:blur(8px)}
.ldc-portal-dialog{display:flex;flex-direction:column;width:min(92vw,440px);max-height:min(88dvh,620px);overflow:hidden;border:1px solid var(--lumiverse-border);border-radius:16px;background:var(--lumiverse-bg-elevated,#111);color:var(--lumiverse-text);box-shadow:0 26px 90px rgba(0,0,0,.65)}
...
.ldc-portal-dialog-body{padding:14px;overflow:auto;-webkit-overflow-scrolling:touch}.ldc-portal-dialog-body .ldc-add-person{padding:0}
...
export function setup(ctx) {
    ...
    function modalDimensions() { const { modalSize, modalExpanded } = uiPreferences(), vw = Math.max(320, window.innerWidth || 1280), vh = Math.max(480, visibleViewportHeight()), mobile = vw <= 720, clampHeight = (target, min, safe) => Math.max(300, Math.min(safe, Math.max(Math.min(min, safe), Math.floor(target)))); let width, contentHeight; if (mobile) {
