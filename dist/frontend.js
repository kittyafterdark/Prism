...
export function setup(ctx) {
    ...
    function wire() {
        ...
        for (const [action, regenerate] of [['setup', false], ['assign', false], ['regenerate', true]])
            modal.root.querySelector(`[data-action=${action}]`)?.addEventListener('click', () => perform(async () => { const r = await request(action === 'setup' ? 'ldc_setup_scene' : 'ldc_assign_colors', { chatId: state.chat.id, regenerate }); acceptState(r.state); render(); }));
    ...
    function candidates() {
        ...
        return all.filter(c => normalizeHex(c.binding?.color)).map(c => { const names = [c.name, ...(c.binding?.aliases || c.aliases || [])], parts = String(c.name || '').split(/\s+/).filter(x => x.length >= 2); for (const part of [parts[0], parts.at(-1)].filter(Boolean)) {
            const owners = all.filter(x => String(x.name || '').split(/\s+/).some(t => t.toLowerCase() === part.toLowerCase()));
            if (owners.length === 1)
                names.push(part);
        } return { key: `character:${c.binding.targetId}`, id: String(c.id), characterId: String(c.characterId || ''), name: c.name, names: [...new Set(names.map(x => String(x || '').trim().toLowerCase()).filter(Boolean))], color: normalizeHex(c.binding.color), binding: c.binding, channels: safeChannels(c.binding), primary: String(c.characterId || '') === String(state?.chat?.characterId || '') }; });
    }
    ...
    async function teach(event, detail, list) {
        ...
        const items = [{ key: 'summary', label: detail.existingColor ? `Existing color: ${detail.existingColor}` : (a.speaker ? `Prism thinks this is ${a.speaker.name}` : 'Prism left this uncolored'), disabled: true }, { key: 'evidence', label: `${a.speaker ? `Speaker: ${a.speaker.name} · ` : ''}${evidenceLabel(a.source)} · ${Math.round(a.confidence * 100)}%`, disabled: true }, { key: 'speaker-head', label: 'Speaker', disabled: true }, { key: 'add-person', label: 'Add new person…' }, ...list.map(c => ({ key: `speaker:${c.key}`, label: c.name, active: c.key === a.speaker?.key })), { key: 'speaker:none', label: 'Keep unassigned', active: !a.speaker }, { key: 'kind-head', label: 'Content type', disabled: true }, { key: 'kind:dialogue', label: 'Dialogue', active: detail.kind === 'dialogue' }, { key: 'kind:thought', label: 'Thought', active: detail.kind === 'thought' }, { key: 'kind:ignored', label: 'Leave unstyled', active: detail.kind === 'ignored' }], result = await ctx.ui.showContextMenu({ position, items }), selected = result.selectedKey;
    ...
    function decorate(span, detail, list) {
        ...
        span.title = a.speaker ? `Assigned to ${a.speaker.name} from ${evidenceLabel(a.source)} · ${Math.round(a.confidence * 100)}%` : (detail.existingColor ? `Existing ${detail.existingColor} · speaker unknown` : 'Prism could not identify this speaker');
