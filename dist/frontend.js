...
export function setup(ctx) {
    ...
    function uiPreferences() { const preferences = state?.preferences || {}, rawLayout = preferences.modalLayout === 'tabs' ? 'horizontal' : preferences.modalLayout; return { modalSize: ['auto', 'compact', 'large'].includes(preferences.modalSize) ? preferences.modalSize : 'auto', modalExpanded: preferences.modalExpanded === true, modalLayout: ['auto', 'split', 'horizontal', 'accessibility'].includes(rawLayout) ? rawLayout : 'auto', showSaveIndicator: preferences.showSaveIndicator !== false }; }
    ...
    function isPhoneLikeViewport() { const coarse = window.matchMedia?.('(pointer: coarse)').matches === true, screenShort = Math.min(Number(screen?.width) || 9999, Number(screen?.height) || 9999), visualWidth = Number(window.visualViewport?.width) || Number(window.innerWidth) || 1280; return coarse && (screenShort <= 900 || visualWidth <= 1100 || window.innerWidth <= 1100); }
    ...
    function modalDimensions() { const { modalSize, modalExpanded } = uiPreferences(), layout = resolvedModalLayout(), vw = Math.max(320, window.innerWidth || 1280), vh = Math.max(480, visibleViewportHeight()), mobile = vw <= 720, horizontal = layout === 'horizontal', split = layout === 'split', clampHeight = (target, min, safe) => Math.max(300, Math.min(safe, Math.max(Math.min(min, safe), Math.floor(target)))); let width, contentHeight; if (mobile) {
        width = Math.floor(vw * .96);
        contentHeight = clampHeight(vh * .84, 500, vh - 84);
    }
        ...
        contentHeight = clampHeight(vh * (mobile ? .88 : .84), mobile ? 560 : 620, vh - (mobile ? 72 : 110));
    }
    else if (split && !modalExpanded && modalSize === 'auto') {
        width = Math.min(Math.floor(vw * .94), 1180);
        contentHeight = clampHeight(Math.min(vh * .74, width * .64), 540, vh - 150);
    } const maxHeight = Math.max(360, Math.min(vh - 24, contentHeight + 96)); return { width: Math.max(360, width), contentHeight, maxHeight, horizontal }; }
    ...
    function render() {
        ...
        const missing = missingCount(), engine = normalizeEngine(state.config.engine), needs = missing > 0, ui = uiPreferences(), expanded = ui.modalExpanded === true, ultrawide = isUltrawideViewport(), modalLayout = resolvedModalLayout(), horizontalLayout = modalLayout === 'horizontal', effectiveUiScale = detectedUiScale();
    ...
    function openAddPerson(seedColor = null) {
        ...
        addModal = resolvedModalLayout() !== 'split' ? createPortalDialog('Add scene character') : ctx.ui.showModal({ title: 'Add scene character', width: 390, maxHeight: 360 });
