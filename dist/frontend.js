...
export function setup(ctx) {
    ...
    function reviewKindLabel(kind) { return ({ 'new-speaker': 'Possible new character', 'unknown-color': 'Unknown colored speaker', 'color-drift': 'Unexpected color for known character', 'speaker-conflict': 'Tag conflicts with explicit prose', 'color-collision': 'Registry color collision', 'alias-suggestion': 'Possible alias', 'known-speaker-color': 'Color found for known character' })[kind] || 'Hybrid observation'; }
    function reviewTarget(value) { const raw = String(value || ''); if (raw.startsWith('speaker:')) {
        const speakerUid = raw.slice(8), character = (state?.characters || []).find(item => item.binding?.speakerUid === speakerUid);
        return character ? { character, binding: character.binding } : null;
    } if (raw.startsWith('character:')) {
        const id = raw.slice(10), character = (state?.characters || []).find(item => String(item.id) === id);
        return character ? { character, binding: character.binding || null } : null;
    } return null; }
    ...
    function reviewPanel() {
        const groups = state?.reviewGroups || [], characters = (state?.characters || []);
        if (!groups.length)
        ...
        const cards = groups.map(group => {
            const matched = reviewBinding(group.matchedSpeakerUid), defaultName = group.inferredName || matched?.name || '', defaultColor = matched ? bindingRegistryColor(matched) : group.observedColor, options = [`<option value="">Create or choose manually</option>`, ...characters.map(character => { const value = character.binding?.speakerUid ? `speaker:${character.binding.speakerUid}` : `character:${character.id}`, selected = (character.binding?.speakerUid === group.matchedSpeakerUid || String(character.id) === String(group.matchedTargetId || '')) ? 'selected' : '', suffix = character.binding?.speakerUid ? '' : ' · unbound'; return `<option value="${esc(value)}" ${selected}>Merge with ${esc(character.name)}${suffix}</option>`; })].join(''), examples = (group.examples || []).map(example => `<span>“${esc(String(example).replace(/^[“”"]|[“”"]$/g, ''))}”</span>`).join('');
    ...
    function wire() {
        ...
        modal.root.querySelectorAll('[data-review-role=merge]').forEach(select => select.addEventListener('change', () => { const target = reviewTarget(select.value), card = select.closest('[data-observation-group]'); if (!target || !card)
            return; const name = card.querySelector('[data-review-role=name]'), color = card.querySelector('[data-review-role=color]'); if (name)
            name.value = target.character.name; if (color && target.binding)
            color.value = bindingRegistryColor(target.binding) || color.value; }));
        modal.root.querySelectorAll('[data-review-action]').forEach(button => button.addEventListener('click', () => perform(async () => { const card = button.closest('[data-observation-group]'), groupKey = card?.dataset.observationGroup; if (!groupKey)
            ...
            const selected = String(card.querySelector('[data-review-role=merge]')?.value || ''), target = reviewTarget(selected);
            payload.mergeSpeakerUid = selected.startsWith('speaker:') ? selected.slice(8) : null;
            if (target && !payload.mergeSpeakerUid) {
                payload.mergeTargetId = String(target.character.id);
                payload.mergeTargetName = target.character.name;
                payload.mergeTargetAliases = target.character.aliases || [];
            }
