...
export function setup(ctx) {
    ...
    function updateEditorPreview() { if (!modal)
        ...
        const line = lines[kind], paint = read.channels[kind].paint, gradient = paint.mode === 'gradient' && paint.stops.length >= 2, value = gradient ? `linear-gradient(${paint.angle}deg,${paint.stops.join(', ')})` : paint.stops[0]; if (line) {
    ...
    } modal.root.querySelectorAll('.ldc-stop span').forEach((stop, index) => stop.style.setProperty('--stop', paint.stops[index] || paint.stops[0])); syncSidebarSwatch(target, read.channels.dialogue.paint); }
