...
function safeChannels(binding) { const color = normalizeHex(binding?.color) || '#B58CFF', raw = binding?.channels, dialogue = { enabled: raw?.dialogue?.enabled !== false, paint: safePaint(raw?.dialogue?.paint, color) }, storedThought = safePaint(raw?.thought?.paint, color), explicit = typeof raw?.thought?.linkedToDialogue === 'boolean' ? raw.thought.linkedToDialogue : null, linked = explicit ?? (!raw?.thought?.paint || paintSignature(storedThought, color) === paintSignature(dialogue.paint, color)); return { dialogue, thought: { enabled: raw?.thought?.enabled === true, linkedToDialogue: linked, paint: linked ? safePaint(dialogue.paint, color) : storedThought } }; }
...
export function setup(ctx) {
    ...
    function readEditorChannels(target) { const channels = safeChannels(target?.binding), stop1 = normalizeHex(modal?.root.querySelector('[data-role=hex]')?.value), mode = modal?.root.querySelector('[data-role=paint-mode]')?.value === 'gradient' ? 'gradient' : 'solid', count = mode === 'gradient' ? (Number(modal?.root.querySelector('[data-role=stop-count]')?.value) || 3) : 1, stop2 = normalizeHex(modal?.root.querySelector('[data-role=hex-2]')?.value) || (stop1 && mode === 'gradient' ? harmonicColor(stop1) : null), stop3 = normalizeHex(modal?.root.querySelector('[data-role=hex-3]')?.value) || stop1, angle = Math.max(0, Math.min(360, Number(modal?.root.querySelector('[data-role=angle]')?.value) || 90)), anchor = normalizeHex(modal?.root.querySelector('[data-role=canonical]')?.value) || normalizeHex(target?.binding?.color) || stop1, stops = mode === 'gradient' ? (count === 3 ? [stop1, stop2, stop3] : [stop1, stop2]).filter(Boolean) : [stop1]; if (!stop1 || !anchor)
        return null; const editedPaint = safePaint({ mode, stops, angle, anchor }, anchor), enabled = modal?.root.querySelector('[data-role=channel-enabled]')?.checked !== false; if (activeChannel === 'thought') {
        const toggle = modal?.root.querySelector('[data-role=thought-linked]'), linked = toggle ? toggle.checked : channels.thought.linkedToDialogue !== false;
        channels.thought = { enabled, linkedToDialogue: linked, paint: linked ? safePaint(channels.dialogue.paint, anchor) : editedPaint };
    }
    else {
        channels.dialogue = { enabled, paint: editedPaint };
        if (channels.thought.linkedToDialogue !== false)
            channels.thought = { ...channels.thought, linkedToDialogue: true, paint: safePaint(editedPaint, anchor) };
    ...
    function updateEditorPreview() { if (!modal)
        ...
        editor.style.setProperty('--editor-gradient', value); modal.root.querySelectorAll('.ldc-stop span').forEach((stop, index) => stop.style.setProperty('--stop', paint.stops[index] || paint.stops[0])); syncSidebarSwatch(target, read.channels.dialogue.paint); }
        ...
        for (const role of ['angle', 'canonical', 'aliases']) {
            ...
            if (input) {
                input.addEventListener('input', () => { if (role === 'angle')
                    markThoughtCustomized(); if (role === 'angle' || role === 'canonical')
                    updateEditorPreview(); queueEditorSave(); });
                input.addEventListener('blur', () => queueEditorSave(true));
            }
        }
        modal.root.querySelector('[data-action=swap-colors]')?.addEventListener('click', () => { markThoughtCustomized(); const hexes = [...modal.root.querySelectorAll('[data-role^=hex]')].filter(x => /^hex(?:-[23])?$/.test(x.dataset.role || '')), pickers = [...modal.root.querySelectorAll('[data-role^=picker]')]; if (hexes.length < 2 || hexes.length !== pickers.length)
