...
export function setup(ctx) {
    ...
    function modalDimensions() { const { modalSize, modalExpanded } = uiPreferences(), vw = Math.max(320, window.innerWidth || 1280), vh = Math.max(480, visibleViewportHeight()), mobile = vw <= 720, horizontal = useHorizontalLayout(), clampHeight = (target, min, safe) => Math.max(300, Math.min(safe, Math.max(Math.min(min, safe), Math.floor(target)))); let width, contentHeight; if (mobile) {
        ...
        contentHeight = clampHeight(Math.min(vh * .78, width * .72), 460, vh - 118);
