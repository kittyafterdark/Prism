...
export function setup(ctx) {
    ...
    function reviewPanel() {
        ...
        const cards = groups.map(group => {
            const matched = reviewBinding(group.matchedSpeakerUid), defaultName = group.inferredName || matched?.name || '', defaultColor = matched ? bindingRegistryColor(matched) : group.observedColor, options = [`<option value="">Create or choose manually</option>`, ...characters.map(character => { const value = character.binding?.speakerUid ? `speaker:${character.binding.speakerUid}` : `character:${character.id}`, selected = character.binding?.speakerUid === group.matchedSpeakerUid ? 'selected' : '', suffix = character.binding?.speakerUid ? '' : ' · unbound'; return `<option value="${esc(value)}" ${selected}>Merge with ${esc(character.name)}${suffix}</option>`; })].join(''), examples = (group.examples || []).map(example => `<span>“${esc(String(example).replace(/^[“”"]|[“”"]$/g, ''))}”</span>`).join('');
