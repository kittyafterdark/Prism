...
.ldc-shell[data-prism-size=compact]{--prism-ui-scale:.96;height:min(78dvh,800px)}
...
export function setup(ctx) {
    ...
    function uiPreferences() { const preferences = state?.preferences || {}; return { modalSize: ['auto', 'compact', 'large'].includes(preferences.modalSize) ? preferences.modalSize : 'auto', modalExpanded: preferences.modalExpanded === true }; }
    ...
    function modalDimensions() { const { modalSize, modalExpanded } = uiPreferences(), vw = Math.max(320, window.innerWidth || 1280), vh = Math.max(480, window.innerHeight || 800), mobile = vw <= 720; let width, maxHeight; if (mobile) {
        ...
        width = Math.min(Math.floor(vw * .92), 980);
        maxHeight = Math.min(Math.floor(vh * .86), 820);
    }
    ...
    function sidebar() {
        ...
        return `<div class="ldc-side-label">Scene roster</div>${rows || '<div class="ldc-empty">No characters added yet.</div>'}${tentative ? `<div class="ldc-side-label ldc-tentative-label">New tagged speakers</div>${tentative}` : ''}<div class="ldc-side-actions"><button class="ldc-btn" data-action="add-person">+ Add character</button><button class="ldc-btn" data-action="assign">Assign missing</button><button class="ldc-btn" data-action="regenerate">Regenerate</button></div>`;
    }
    ...
    function render() {
        ...
        const missing = missingCount(), engine = normalizeEngine(state.config.engine), needs = missing > 0, ui = uiPreferences(), expanded = ui.modalExpanded === true, ultrawide = isUltrawideViewport();
    ...
    function wire() {
        ...
        modal.root.querySelector('[data-action=toggle-expanded]')?.addEventListener('click', () => perform(async () => { const next = !uiPreferences().modalExpanded, response = await request('ldc_update_options', { modalExpanded: next }, 15000); acceptState(response.state); await reopenPalette(); }));
        modal.root.querySelector('[data-role=modal-size]')?.addEventListener('change', e => perform(async () => { const response = await request('ldc_update_options', { modalSize: e.target.value }, 15000); acceptState(response.state); await reopenPalette(); }));
        modal.root.querySelector('[data-role=modal-expanded]')?.addEventListener('change', e => perform(async () => { const response = await request('ldc_update_options', { modalExpanded: e.target.checked }, 15000); acceptState(response.state); await reopenPalette(); }));
    ...
    async function reopenPalette() { const previousSide = sideScroll, previousPanel = panelScroll; await openPalette(false); sideScroll = previousSide; panelScroll = previousPanel; if (modal)
        render(); }
