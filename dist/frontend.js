...
export function setup(ctx) {
    ...
    function missingCount() { return (state?.characters || []).filter(c => !normalizeHex(c.binding?.color)).length + (state?.config?.personaEnabled !== false && state?.persona && !normalizeHex(state.persona.binding?.color) ? 1 : 0); }
